# @omegion1npm/corrupti-labore-minima

![CI](https://github.com/omegion1npm/corrupti-labore-minima/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@omegion1npm/corrupti-labore-minima.svg?style=flat)](https://www.npmjs.com/package/@omegion1npm/corrupti-labore-minima)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@omegion1npm/corrupti-labore-minima.svg?style=flat)](https://www.npmjs.com/package/@omegion1npm/corrupti-labore-minima)


__@omegion1npm/corrupti-labore-minima__ is significantly faster than `JSON.stringify()` for small payloads.
...
@omegion1npm/corrupti-labore-minima requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@omegion1npm/corrupti-labore-minima array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@omegion1npm/corrupti-labore-minima array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@omegion1npm/corrupti-labore-minima large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@omegion1npm/corrupti-labore-minima large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@omegion1npm/corrupti-labore-minima long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@omegion1npm/corrupti-labore-minima short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@omegion1npm/corrupti-labore-minima obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@omegion1npm/corrupti-labore-minima date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@omegion1npm/corrupti-labore-minima">https://runkit.com/npm/@omegion1npm/corrupti-labore-minima</a>
## Example

```js
const fastJson = require('@omegion1npm/corrupti-labore-minima')
...
Optionally, you may provide to `@omegion1npm/corrupti-labore-minima` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@omegion1npm/corrupti-labore-minima` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@omegion1npm/corrupti-labore-minima` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@omegion1npm/corrupti-labore-minima` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@omegion1npm/corrupti-labore-minima` supports pattern properties as defined by JSON schema.
...
`@omegion1npm/corrupti-labore-minima` supports additional properties as defined by JSON schema.
...
`@omegion1npm/corrupti-labore-minima` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@omegion1npm/corrupti-labore-minima` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@omegion1npm/corrupti-labore-minima`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@omegion1npm/corrupti-labore-minima` to handle arrays starts increasing exponentially, leading
...
@omegion1npm/corrupti-labore-minima large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@omegion1npm/corrupti-labore-minima large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@omegion1npm/corrupti-labore-minima`
...
Users are responsible for sending trusted data. `@omegion1npm/corrupti-labore-minima` guarantees that you will get
...
itself. You need to have `@omegion1npm/corrupti-labore-minima` installed for the standalone code to work.
